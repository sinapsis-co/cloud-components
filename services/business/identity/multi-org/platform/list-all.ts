...
export const listAllOrgWorkspace = async (orgId: string, nextToken?): Promise<OrgWorkspace[]> => {
  ...
  const { items, nextToken: nextTokenResponse } = await repoOrgWorkspace.listItem(orgId, { limit: 1000, nextToken });
  allItems.push(...items);
...
export const listAllOrgUser = async (orgId: string, nextToken?): Promise<OrgUser[]> => {
  ...
  const { items, nextToken: nextTokenResponse } = await repoOrgUser.listItem(orgId, { limit: 1000, nextToken });
  allItems.push(...items);
  if (nextTokenResponse) {
