...
>(
  ...
  try {
    ...
    const cpRecipeBucket = `aws s3 cp ${distDir}/_next s3://${recipeBucket}/ --recursive --cache-control "max-age=${indexMaxAge}" --exclude '.next/_next/cache/*'  --exclude '.next/_next/static/*'`;
